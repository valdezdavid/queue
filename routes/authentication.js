...
module.exports = (router) => {
                            ...
                            if (err.errors.password) {
                              res.json({ success: false, message: err.errors.password.message }); // Return error
                                ...
                                if(err.errors.student){
                                  ...
                                  if(err.errors.professor) {
                                    res.json({ success: false, message: err.errors.profe.message })
                                  }
                                  else {
  ...
  router.get('/profile', (req, res) => {
    ...
    User.findOne({ _id: req.decoded.userId }).select('username email student professor').exec((err, user) => {
  ...
  router.get('/info', (req, res) => {
    // Search for user in database
    User.findOne({ _id: req.decoded.userId }).select('student professor').exec((err, user) => {
      // Check if error connecting
      if (err) {
        res.json({ success: false, message: err }); // Return error
      } else {
  ...
  /* ================================================
 GETS ALL THE PROFESSORS/TAS
  ================================================ */
  router.get('/allProfessors', (req,res) => {
    User.find({'professor': 'true'}, (err, professors) => {
      if(err){
        res.json({success: false, message: err});
      }
      else{
...
/* ================================================
  UPDATE LOCATION ROUTE FOR PROFESSORS
  ================================================ */
  ...
  User.findOne({_id: req.decoded.userId}).select('username email').exec((err,user) => {
    if(err){
      ...
      if(!user){
        ...
        user.posx = req.body.posx;
        user.posy = req.body.posy;
        user.save((err) => {
          if(err){
            res.json({success: false, message: err});
          }
